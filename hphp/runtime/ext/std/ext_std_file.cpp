...
Variant HHVM_FUNCTION(fopen, const String& filename, const String& mode,
...
Variant HHVM_FUNCTION(pclose, const Resource& handle) {
  CHECK_HANDLE(handle, f);
  CHECK_ERROR(f->close());
  return s_file_data->m_pcloseRet;
}

Variant HHVM_FUNCTION(fseek, const Resource& handle, int64_t offset,
...
Variant HHVM_FUNCTION(fscanf, const Resource& handle, const String& format,
  ...
  return HHVM_FN(sscanf)(args.size(), line, format, args);
}
...
Variant HHVM_FUNCTION(fputs, const Resource& handle, const String& data, int64_t length /* = 0 */) {
  CHECK_HANDLE(handle, f);
  int64_t ret = f->write(data, length);
  if (ret < 0) ret = 0;
  return ret;
...
Variant HHVM_FUNCTION(fprintf, const Resource& handle, const String& format,
                  const Array& args /* = null_array */) {
  CHECK_HANDLE(handle, f);
  return f->printf(format, args);
}

Variant HHVM_FUNCTION(vfprintf, const Resource& handle, const String& format, const Array& args) {
  CHECK_HANDLE(handle, f);
  return f->printf(format, args);
}
...
bool HHVM_FUNCTION(flock, const Resource& handle, int operation, VRefParam wouldblock /* = null */) {
  CHECK_HANDLE(handle, f);
  bool block = false;
  int act;
...
Variant HHVM_FUNCTION(fputcsv, const Resource& handle, const Array& fields,
...
Variant HHVM_FUNCTION(fgetcsv, const Resource& handle, int64_t length /* = 0 */,
                  const String& delimiter /* = "," */,
                  const String& enclosure /* = "\"" */,
                  const String& escape /* = "\\" */) {
  if (length < 0) {
...
Variant HHVM_FUNCTION(file_get_contents, const String& filename,
                            bool use_include_path /* = false */,
                            const Variant& context /* = null */,
                            int64_t offset /* = -1 */,
                            int64_t maxlen /* = -1 */) {
  CHECK_PATH(filename, 1);
  Variant stream = HHVM_FUNCTION(fopen, filename, "rb", use_include_path, context);
  if (same(stream, false)) return false;
  return HHVM_FUNCTION(stream_get_contents, stream.toResource(), maxlen, offset);
}

Variant HHVM_FUNCTION(file_put_contents, const String& filename, const Variant& data,
                            int flags /* = 0 */,
                            const Variant& context /* = null */) {
  CHECK_PATH(filename, 1);
...
Variant HHVM_FUNCTION(file, const String& filename, int flags /* = 0 */,
               const Variant& context /* = null */) {
  CHECK_PATH(filename, 1);
...
Variant HHVM_FN(parse_ini_file)(const String& filename,
  ...
  if (translated.empty() || !HHVM_FN(file_exists)(translated)) {
    if (filename[0] != '/') {
  ...
  Variant content = HHVM_FN(file_get_contents)(translated);
  if (same(content, false)) return false;
  return IniSetting::FromString(content.toString(), filename, process_sections,
                                scanner_mode);
}

Variant HHVM_FUNCTION(parse_ini_string, const String& ini,
                           bool process_sections /* = false */,
                           int scanner_mode /* = k_INI_SCANNER_NORMAL */) {
  return IniSetting::FromString(ini, "", process_sections, scanner_mode);
}

Variant HHVM_FUNCTION(md5_file, const String& filename, bool raw_output /* = false */) {
  CHECK_PATH(filename, 1);
  return HHVM_FN(hash_file)("md5", filename, raw_output);
}

Variant HHVM_FUNCTION(sha1_file, const String& filename, bool raw_output /* = false */) {
  CHECK_PATH(filename, 1);
...
bool HHVM_FUNCTION(is_writeable, const String& filename) {
  CHECK_PATH_FALSE(filename, 1);
  return HHVM_FUNCTION(is_writable, filename);
}
...
Variant HHVM_FUNCTION(disk_free_space, const String& directory) {
  ...
  return (double)buf.HHVM_FUNCTION(bsize * , double)buf.HHVM_FUNCTION(bavail;
}
...
Variant HHVM_FUNCTION(disk_total_space, const String& directory) {
  ...
  String translated = File::TranslatePath(directory);
  CHECK_SYSTEM(statfs(translated.c_str(), &buf));
  return (double)buf.HHVM_FUNCTION(bsize * , double)buf.HHVM_FUNCTION(blocks;
}
...
bool HHVM_FUNCTION(touch, const String& filename, int64_t mtime /* = 0 */,
             int64_t atime /* = 0 */) {
  CHECK_PATH_FALSE(filename, 1);
...
int64_t HHVM_FUNCTION(umask, const Variant& mask /* = null_variant */) {
...
bool HHVM_FUNCTION(unlink, const String& filename, const Variant& context /* = null */) {
  CHECK_PATH_FALSE(filename, 1);
  Stream::Wrapper* w = Stream::getWrapperFromURI(filename);
  if (!w) return false;
...
String HHVM_FUNCTION(basename, const String& path,
...
bool HHVM_FUNCTION(fnmatch, const String& pattern,
...
bool HHVM_FUNCTION(rmdir, const String& dirname, const Variant& context /* = null */) {
...
bool HHVM_FUNCTION(chdir, const String& directory) {
  ...
  if (HHVM_FUNCTION(is_dir, directory)) {
...
Variant HHVM_FUNCTION(opendir, const String& path, const Variant& context /* = null */) {
  Stream::Wrapper* w = Stream::getWrapperFromURI(path);
  if (!w) return false;
...
Variant HHVM_FUNCTION(readdir, const Resource& dir_handle /* = null */) {
...
void HHVM_FUNCTION(rewinddir, const Resource& dir_handle /* = null */) {
  Directory *dir = get_dir(dir_handle);
  if (!dir) {
    return;
  }
...
void HHVM_FUNCTION(closedir, const Resource& dir_handle /* = null */) {
...
//  Native::registerConstant<KindOfResource>(makeStaticString("STDIN"), k_STDIN);
//  Native::registerConstant<KindOfResource>(makeStaticString("STDOUT"), k_STDOUT);
//  Native::registerConstant<KindOfResource>(makeStaticString("STDERR"), k_STDERR);
  Native::registerConstant<KindOfInt64>(makeStaticString("STREAM_URL_STAT_LINK"), k_STREAM_URL_STAT_LINK);
  Native::registerConstant<KindOfInt64>(makeStaticString("STREAM_URL_STAT_QUIET"), k_STREAM_URL_STAT_QUIET);
